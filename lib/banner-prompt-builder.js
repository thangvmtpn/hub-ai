...
export function buildBannerPrompt(data = {}, visualAssets = []) {
  ...
  // Determine actual visual subject dynamically
  let rawSubject = visual_elements || custom_topic || content_group || product_line || 'Sản phẩm trà Việt Nam cao cấp';
  const translatedVisuals = translateVisualText(rawSubject);
  const translatedStyle = translateVisualText(ref_style || 'Phong cách hiện đại, bố cục cân đối, ánh sáng tự nhiên');
  const translatedHeadlineColor = translateVisualText(headline_color || 'Màu vàng gold');
  const translatedHeadlineEffect = translateVisualText(headline_effect || 'Hiệu ứng chữ cổ dạng viết tay');
  ...
  const headlineSection = headline
    ? `Typography & Title: Includes headline text "${headline}" in ${translatedHeadlineColor}, styled with ${translatedHeadlineEffect}.`
    : '';
