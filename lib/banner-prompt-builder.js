...
export function buildBannerPrompt(data = {}, visualAssets = []) {
  ...
  const pro3Instruction = [
    'Edit Instruction: Modify specified banner components while preserving overall background structure and coherence.',
    edit_target ? `Target Component to Edit: ${toEnglish(edit_target)}.` : '',
    edit_instructions ? `Detailed Editing Instructions: "${toEnglish(edit_instructions)}".` : '',
    ref_image ? `Original Image Reference: Apply edits based on reference image (${ref_image}).` : ''
  ].filter(Boolean).join(' ');

  const pro4Instruction = [
    'Resize & Recompose Instruction: Adapt banner layout to new target dimensions while preserving key brand elements.',
    target_aspect_ratio ? `Target Aspect Ratio / Format: ${toEnglish(target_aspect_ratio)}.` : '',
    resize_notes ? `Layout Recomposition Notes: "${toEnglish(resize_notes)}".` : '',
    ref_image ? `Original Image Reference: Recompose from original reference image (${ref_image}).` : ''
  ].filter(Boolean).join(' ');
  ...
  const objectiveMap = {
    ...
    PRO3: {
      opener: 'An edited version of an existing banner image. Apply specific component modifications.',
      productInstruction: pro3Instruction,
    },
    PRO4: {
      opener: 'Resize and recompose the banner image to fit a new aspect ratio and layout constraint.',
      productInstruction: pro4Instruction,
    },
  };
