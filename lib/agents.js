...
export const AGENTS = [
  ...
  },
  {
    ...
    fields: [
      { key: 'prompt_objective', label: '🎯 Mục tiêu Prompt', type: 'select', required: true,
        options: [
          'PRO1 — Làm banner ảnh có hình sản phẩm',
          'PRO2 — Làm banner ảnh không hình sản phẩm',
          'PRO3 — Chỉnh sửa thành phần (Màu sắc, nội dung, bố cục,...)',
          'PRO4 — Thay đổi kích thước, tỉ lệ của ảnh'
        ] },
      { key: 'edit_target', label: '🔧 PRO3: Thành phần cần chỉnh sửa', type: 'select', showIf: 'PRO3',
        options: [
          'Đổi màu sắc chủ đạo / màu nền',
          'Thay đổi nội dung text / Tiêu đề / Font chữ',
          'Di chuyển / Sắp xếp lại vị trí bố cục',
          'Thay đổi / Thêm bớt vật thể minh họa',
          'Chỉnh sửa tông màu & ánh sáng'
        ] },
      { key: 'edit_instructions', label: '📝 PRO3: Chi tiết yêu cầu chỉnh sửa', type: 'textarea', showIf: 'PRO3',
        placeholder: 'VD: Đổi màu phông nền từ xanh lá sang đỏ ruby, thay tiêu đề thành "Trà Dược Cao Cấp", giữ nguyên vị trí hộp quà...' },
      { key: 'target_aspect_ratio', label: '📐 PRO4: Tỉ lệ / Kích thước ảnh mới', type: 'select', showIf: 'PRO4',
        options: [
          '1:1 — Vuông (Facebook Post / Instagram Feed)',
          '9:16 — Dọc (Story / Reel / TikTok)',
          '16:9 — Ngang (Website Banner / Youtube Cover)',
          '4:5 — Dọc chuẩn (Facebook / Instagram Feed)',
          '3:2 — Ngang chuẩn (Landscape Banner)',
          '2:1 — Banner rộng (Header Cover)'
        ] },
      { key: 'resize_notes', label: '📝 PRO4: Ghi chú sắp xếp lại bố cục', type: 'textarea', showIf: 'PRO4',
        placeholder: 'VD: Mở rộng phông nền đồi chè sang 2 bên, điều chỉnh lại khoảng trống cho cân đối với tỉ lệ mới...' },
      ...
      { key: 'headline', label: '1. Text tiêu đề ảnh (tuỳ chọn)', type: 'text',
        placeholder: 'VD: Những điều thú vị về trà xanh Thái Nguyên' },
      { key: 'headline_color', label: '2. Màu sắc text tiêu đề', type: 'select',
        options: [
          'Màu vàng gold (Golden yellow)',
          'Màu như ảnh banner tham khảo',
          'Màu đỏ ruby sang trọng',
          'Màu trắng tinh tế',
          'Màu xanh ngọc trà'
        ] },
      { key: 'headline_effect', label: '3. Hiệu ứng đặc biệt text tiêu đề', type: 'select',
        options: [
          'Hiệu ứng chữ cổ dạng viết tay (Vintage calligraphic script)',
          'Như ảnh banner tham khảo',
          'Hiệu ứng mạ vàng ánh kim (Metallic gold embossing)',
          'Hiệu ứng 3D đổ bóng sang trọng',
          'Hiệu ứng phát sáng nghệ thuật (Soft ambient glow)'
        ] },
      { key: 'visual_elements', label: '4. Hình ảnh minh họa chi tiết', type: 'textarea',
        placeholder: 'VD: - Hình ảnh đồi chè xanh Thái Nguyên\n- Hình ảnh người đàn ông trung niên đang nhâm nhi chén trà ngoài sân' },
      { key: 'subtext', label: '5. Nội dung text phụ (nếu có)', type: 'textarea',
        placeholder: 'VD: Đậm đà hương vị truyền thống Việt Nam' },
      { key: 'extra_text', label: '6. Thông tin khác trên ảnh', type: 'text',
        placeholder: 'VD: Logo Trà Dược Việt Nam / Hoặc ghi "Không có"' },
      { key: 'primary_color', label: '7. Mã màu / Màu sắc chủ đạo', type: 'text',
        placeholder: 'VD: #1B4332 (Xanh đồi chè)' },
      { key: 'secondary_color', label: '8. Mã màu / Màu sắc phụ', type: 'text',
        placeholder: 'VD: #D4AF37 (Vàng kim)' },
      { key: 'custom_topic', label: '9. Ghi chú / chủ đề cụ thể (tuỳ chọn)', type: 'textarea',
        placeholder: 'VD: Viết về lợi ích polyphenol trong trà xanh, hoặc so sánh Móc Câu vs Nõn Tôm...' }
    ],
  },
  {
    id: 'biz_banner_designer', department: 'business',
    name: 'Thiết Kế Banner & Visual AI', icon: 'palette',
    desc: 'Tạo Prompt chuẩn 9 thành phần thiết kế + vẽ Banner AI thương mại chuyên nghiệp',
    type: 'content-creator',
    fields: [
      { key: 'prompt_objective', label: '🎯 Mục tiêu Prompt', type: 'select', required: true,
        options: [
      ...
      { key: 'resize_notes', label: '📝 PRO4: Ghi chú sắp xếp lại bố cục', type: 'textarea', showIf: 'PRO4',
        placeholder: 'VD: Mở rộng phông nền đồi chè sang 2 bên, điều chỉnh lại khoảng trống cho cân đối với tỉ lệ mới...' },
      { key: 'ref_image', label: '🖼 PRO3/PRO4: Tải lên hoặc dán link ảnh gốc', type: 'image_upload', showIf: 'PRO3,PRO4',
        placeholder: 'Chọn ảnh từ máy tính hoặc dán URL ảnh gốc...' },
      { key: 'product_line', label: 'Dòng sản phẩm', type: 'select', required: true,
        options: [
          'Vạn Lộc Trà — Trà xanh Thái Nguyên',
          'Vạn Thọ Trà — Trà Dược',
          ...
          'TRABA — Bánh ăn cùng trà',
          'Vạn Thịnh Trà — Trà biếu',
          'Tất cả — Sản phẩm trà chung'
        ] },
      { key: 'headline', label: '1. Text tiêu đề chính', type: 'text', required: true,
        placeholder: 'VD: Những điều thú vị về trà xanh Thái Nguyên' },
      { key: 'headline_color', label: '2. Màu sắc text tiêu đề', type: 'select',
        options: [
          'Màu vàng gold (Golden yellow)',
      ...
      { key: 'ref_style', label: '4. Tham khảo phong cách & bố cục', type: 'select',
        options: [
          'Phong cách thương mại cao cấp, bố cục đối xứng, ánh sáng ấm',
          'Nghệ thuật trà Việt truyền thống, bố cục tối giản (Minimalism)',
          'Infographic hiện đại, bố cục chia mảng thông tin rõ ràng',
          'Nhiếp ảnh sản phẩm đồi chè, góc máy chiều sâu nghệ thuật'
        ] },
      { key: 'visual_elements', label: '5. Hình ảnh minh họa chính', type: 'textarea', required: true,
        placeholder: 'VD: - Hình ảnh đồi chè xanh Thái Nguyên\n- Hình ảnh người đàn ông trung niên đang nhâm nhi chén trà ngoài sân' },
      { key: 'subtext', label: '6. Nội dung text phụ (nếu có)', type: 'textarea',
        placeholder: 'VD: Đậm đà hương vị truyền thống Việt Nam' },
      { key: 'extra_text', label: '7. Thông tin khác trên ảnh', type: 'text',
        placeholder: 'VD: Logo Trà Dược Việt Nam / Hoặc ghi "Không có"' },
      { key: 'primary_color', label: '8. Mã màu / Màu sắc chủ đạo', type: 'text',
        placeholder: 'VD: #1B4332 (Xanh đồi chè) hoặc Màu xanh trà sẫm' },
      { key: 'secondary_color', label: '9. Mã màu / Màu sắc phụ', type: 'text',
        placeholder: 'VD: #D4AF37 (Vàng kim) hoặc Vàng nhạt' },
    ],
  },
  {
